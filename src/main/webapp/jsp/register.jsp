        	...
        	<div align="center">
        		<center><form method="post" action="serviceCentre">
        			<fieldset  style="width:20%;">
        				...
        				<input type="text" name="firstName" pattern="[A-ZА-яа-яa-z]{2,20} " class="form-control" id="fName" required>
        				<br><fmt:message key="registration.lastname" bundle="${locale}"/><br>
        				<input type="text" name="lastName" pattern="[A-ZА-яа-яa-z]{2,30}" class="form-control" required>
        				<br><fmt:message key="registration.email" bundle="${locale}"/><br>
        				<input type="email" pattern="[a-z0-9._%+-]+@[a-z0-9.-]+\.[a-z]{2,3}$" class="form-control" id="email" name="email" required>
        				<br><fmt:message key="registration.phone" bundle="${locale}"/><br>
        				<input type="text" id="phone"  name="phone" class="form-control" required>
        				<br>
        				<fmt:message key="registration.login" bundle="${locale}"/><br>
        				<input type="text" name="login" pattern="[A-ZА-яа-яa-z0-9]{4,20}" class="form-control" required>
        				<br>
        				<fmt:message key="registration.password" bundle="${locale}"/><br>
        				<input type="password" id="password" name="password" pattern="[A-ZА-яа-яa-z0-9]{4,30}" class="form-control" required>
        				<br>
        				<fmt:message key="registration.repeat.password" bundle="${locale}"/><br>
        				<input type="password" id="password_confirm" name="password_confirm" pattern="[A-ZА-яа-яa-z0-9]{4,30}" class="form-control" required>
        				<br>
        				<br>
